...
def get_dataset(name: str) -> pd.DataFrame:
    ...
    if not os.path.exists(file_path):
        valid_datasets = [f.split('.')[0] for f in os.listdir(data_dir) if f.endswith('.csv')]
        raise FileNotFoundError(f"Dataset '{name}' not found. Available datasets: {', '.join(valid_datasets)}")
