...
import numpy as np
from pygifi._linalg import ls_rc
...
def pava(xo, wo=None):
    ...
    i = 0         # current block index (0-indexed)

    while i < m:
        if i < m - 1 and x[i] > x[i + 1]:
            # Pool UPWARD: merge block i with block i+1
            ww = w[i] + w[i + 1]
            if ww > 0:
            ...
            if m == 1:
                break
            # Back-check: might need to pool downward now
            if i > 0 and x[i - 1] > x[i]:
                i -= 1
        elif i > 0 and x[i - 1] > x[i]:
            # Pool DOWNWARD: merge block i-1 with block i
            ww = w[i - 1] + w[i]
            if ww > 0:
...
def isotone(x, y, w=None, ties='s'):
    ...
    if ties == 's':
        ...
        result_pool = np.zeros(len(there))
        result_pool[np.argsort(ordered_idx)] = r
        s[there] = result_pool
        s[notthere] = y[notthere]
