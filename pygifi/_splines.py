...
def bspline_basis(x, degree, innerknots, lowknot=None, highknot=None):
    """
...
def knots_gifi(x, type='Q', n=None):
    """
    ...
    type : str, one of:
        ...
        'R' — Regular equally-spaced knots
        'E' — No interior knots (empty list per column)
        'D' — Knots at unique data values (drop min/max)
    n : int or None, default=None
        Number of interior knots (before endpoint removal).
        If None, dynamically matches R's max(1, len(unique) // 3).

    Returns
    -------
    list of np.ndarray — one knot array per column.
    """
